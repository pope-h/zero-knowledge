...
pub fn compute_commitment<F: PrimeField, P: Pairing>(poly: &MultiLinearPoly<F>, encrypted_basis: &[P::G1]) -> P::G1 {
...
pub fn compute_poly_minus_v<F: PrimeField>(mut poly: MultiLinearPoly<F>, vars_to_open: &[F]) -> MultiLinearPoly<F> {
...
pub fn compute_quotient<F: PrimeField>(poly: &MultiLinearPoly<F>) -> MultiLinearPoly<F> {
    ...
    MultiLinearPoly { computation: quotient }
}

pub fn compute_remainder<F: PrimeField>(poly: MultiLinearPoly<F>, eval_point: F) -> MultiLinearPoly<F> {
...
pub fn blow_up<F: PrimeField>(poly: MultiLinearPoly<F>, blow_up_times: usize) -> MultiLinearPoly<F> {
    ...
    for _i in 0..blow_up_times {
        poly = MultiLinearPoly { computation: explode(poly) };
    }
...
#[cfg(test)]
pub mod test {
    use crate::{multi_linear::MultiLinearPoly, kzg::trusted_setup::tests::setup};
    ...
    pub fn poly_1() -> MultiLinearPoly<BlsFr> {
        // f(x) = 3ab + 4c
        MultiLinearPoly::new(&vec![BlsFr::from(0), BlsFr::from(4), BlsFr::from(0), BlsFr::from(4), BlsFr::from(0), BlsFr::from(4), BlsFr::from(3), BlsFr::from(7)])
    }
    ...
    fn test_compute_quotient() {
        ...
        let expected = MultiLinearPoly::new(&vec![BlsFr::from(0), BlsFr::from(0), BlsFr::from(3), BlsFr::from(3)]);
    ...
    fn test_test_compute_poly_minus_v() {
        ...
        let new_poly = vec!(BlsFr::from(-72), BlsFr::from(-68), BlsFr::from(-72), BlsFr::from(-68), BlsFr::from(-72), BlsFr::from(-68), BlsFr::from(-69), BlsFr::from(-65));
    ...
    fn test_compute_remainder() {
        ...
        let remainder = vec!(BlsFr::from(-72), BlsFr::from(-68), BlsFr::from(-54), BlsFr::from(-50));
    ...
    fn test_blow_up() {
        ...
        let new_poly = vec!(BlsFr::from(3), BlsFr::from(4), BlsFr::from(3), BlsFr::from(4));
        assert_eq!(new_poly, result.computation);
    }
}